...
from code_factory.workspace.review.review_ports import (
    _listening_pid,
    _process_command,
...
def test_ensure_review_port_available_covers_free_and_conflict_paths(
    ...
    monkeypatch.setattr(
        "code_factory.workspace.review.review_ports.review_port_in_use",
    ...
    monkeypatch.setattr(
        "code_factory.workspace.review.review_ports.review_port_owner_details",
        lambda port: None,
    )
...
def test_review_host_and_port_probe_helpers(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "code_factory.workspace.review.review_ports.socket.socket",
...
def test_review_port_owner_details_and_process_helpers(
    ...
    monkeypatch.setattr(
        "code_factory.workspace.review.review_ports._listening_pid", lambda port: None
    )
    assert review_port_owner_details(8000) is None

    monkeypatch.setattr(
        "code_factory.workspace.review.review_ports._listening_pid", lambda port: 123
    )
    monkeypatch.setattr(
        "code_factory.workspace.review.review_ports._process_command", lambda pid: None
    )
    assert review_port_owner_details(8000) == "Occupying PID: 123."

    monkeypatch.setattr(
        "code_factory.workspace.review.review_ports._process_command",
    ...
    monkeypatch.setattr(
        "code_factory.workspace.review.review_ports.subprocess.run", raise_oserror
    )
    assert _listening_pid(8000) is None
    assert _process_command(123) is None

    monkeypatch.setattr(
        "code_factory.workspace.review.review_ports.subprocess.run",
