...
RUNNING_TABLE_COLUMNS: tuple[dict[str, Any], ...] = (
    ...
    {"header": "RETRY", "width": 5, "style": "yellow", "justify": "right"},
    {"header": "REPAIR", "width": 6, "style": "yellow", "justify": "right"},
...
def _running_renderable(running: list[dict[str, Any]]) -> RenderableType:
    ...
    for entry in sorted(running, key=lambda item: str(item.get("identifier") or "")):
        ...
        table.add_row(
            ...
            Text(clean_inline(entry.get("activity_phase") or "Execution", 14)),
            Text(clean_inline(entry.get("runtime_pid") or "n/a", 8), style="yellow"),
            Text(_runtime_and_turns(entry), style="magenta"),
            Text(str(int_value(entry.get("worker_retries"))), style="yellow"),
            Text(str(int_value(entry.get("repair_attempts"))), style="yellow"),
